...
async function handleCustomerSignup(event) {
    ...
    const signupData = {
        ...
        full_name: formData.full_name, // Keep for localStorage fallback
        address: formData.address
    };

    try {
        showLoading();
        await Auth.signupCustomer(signupData);
        hideLoading();
...
async function handleBuilderSignup(event) {
    ...
    // Prepare data for backend API
    const signupData = {
        firstName: formData.company_name, // Use company name as first name
        lastName: 'Builder', // Default last name for builders
        email: formData.email,
        phone: formData.phone,
        password: formData.password,
    ...
    try {
        showLoading();
        await Auth.signupBuilder(signupData);
        hideLoading();
